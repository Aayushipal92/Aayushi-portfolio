...
/** 0→1 progress matching spiral circle radius growth */
export function getSpiralCircleProgress(time: number): number {
  const t1 = constrain(map(time, 0, SPIRAL_GROWTH_END, 0, 1), 0, 1)
  let p = constrain(1.2 * t1, 0, 1)
  p = ease(p, 1.8)
  return Math.sqrt(p)
}
...
class AnimationController {
  ...
  private readonly changeEventTime = CHANGE_EVENT_TIME
  ...
  render() {
    ...
    this.drawTrail(t1, particleColor)
...
export function SpiralAnimation({ onProgress }: SpiralAnimationProps) {
  ...
  useEffect(() => {
    ...
    animationRef.current = new AnimationController(canvas, ctx, dpr, size, (time) =>
